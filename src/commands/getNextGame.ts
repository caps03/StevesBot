...
export class GetNextGame implements Command {
    ...
    public async execute(intr: ChatInputCommandInteraction): Promise<void> {
        ...
        if(sport != null && sport != undefined) {
            if(main.Sport.Teams.filter(t => t.Sport == sport && t.Name == teamName).length > 0) {
                team = main.Sport.Teams.filter(t => t.Sport == sport && t.Name == teamName)[0];
            }
        } else {
        ...
        if (team != null && team != undefined) {
            if (team.NextEvent.id != "") {
                await InteractionUtils.send(intr, team.Name + "'s next game is " + team.NextEvent.date);
            } else {
