...
export class Movie implements Command {
  ...
  public async execute(intr: ChatInputCommandInteraction): Promise<void> {
    ...
    let movieSearch = await axios.get<any>(
        Config.APIKeys.TheMovieDB.Url + "3/search/movie?api_key=" + Config.APIKeys.TheMovieDB.V3.Key + "&query=" + movie,
        {
          headers: {
    ...
    if (movieSearch?.data?.results?.length > 0) {
        ...
        let movieData = await axios.get<any>(
            Config.APIKeys.TheMovieDB.Url + "3/movie/" + movieId + "?api_key=" + Config.APIKeys.TheMovieDB.V3.Key,
            {
              headers: {
                Accept: "application/json",
              },
        ...
        let posterPath: string = "https://www.themoviedb.org/t/p/w300_and_h450_bestv2/" + movieSearch?.data?.results[0].poster_path;
        ...
        let backdropPath: string = "https://www.themoviedb.org/t/p/w533_and_h300_bestv2/" + movieSearch?.data?.results[0].backdrop_path;
        let genres: string = "";
        ...
        if(movieData?.data?.id == movieId) {
            ...
            imdbLink = "https://www.imdb.com/title/" + movieData?.data?.imdb_id + "/";
        } else {
